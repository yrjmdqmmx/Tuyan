...
验证证据暂存于 `/Users/a1-6/.codex/tmp/tuyan-miniprogram-watcha-20260910/`；所有原生截图文件名包含 `mocked`。上游观猹、邮件、支付及账号删除在本地测试使用替身，不操作真实用户身份。
...
- 本地上传副本采用上一版本的 192 项 manifest 做基线校验，再只同步受控文件；保存所有本机额外文件，并重新计算两侧 checksum。
