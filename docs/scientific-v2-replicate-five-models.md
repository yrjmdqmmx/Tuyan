...
`remediate-freeze` 可以从当前有效的单模型扩展发布派生 `slotRetest` 批次，
目前只接受一个非 canary 的 Replicate 题位：原批次必须已终结，目标题必须是
已确认失败且有账单核对的零实扣记录。未知结果、成功题、未核对费用、多个目标均拒绝。
新批次仍保持 Replicate 每题最多一次调用；提示词、路由、输出参数及冻结价格不变。

Core 和 Worker 把来源发布、目标题及目标集合哈希绑定进新 manifest。
其余八题的终态和调用记录被核验继承，只有目标题进入新 A/B 独立盲审、争议仲裁和图片渲染。
发布时逐字段保留其余八题与所有其他模型，只重新计算该模型聚合分数及全榜的派生排名。
旧失败发布和调用记录不可变，新发布记录 `slotRetest.source`，费用沿来源链查询。
成功重测的新费用经账单核对后追加独立证据，原零费用记录保留。
