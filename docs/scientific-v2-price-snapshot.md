...
MAI-Image-2.6 has a separate exact-model conservative allowance. Refresh captures both its Dedicated Image API endpoint (Azure input text/image/output image at USD 5/8/38 per million tokens) and its ordinary model endpoint (token ceilings). The fixed allowance uses 32,000 text tokens, another 32,000 image tokens for edits, and at most 1,024 output tokens, converted at USD × 8: CNY 1.591296 per generation and 3.639296 per edit. The larger input ceiling is conservative relative to the current OpenRouter catalog and matches Microsoft's documented maximum context; it is not a claim about actual tokens or output pixels. A changed price, an additional charge, missing/raised token ceiling, wrong model, or missing Azure route rejects authorization without falling back to the old allowance. For the nine new-model slots the estimated upper-bound total is CNY 20.465664 (CNY 81.862656 for four attempts each). Report this as an allowance, never as measured billing. Source: [Microsoft MAI API parameters](https://learn.microsoft.com/en-us/azure/foundry/foundry-models/how-to/use-foundry-models-mai-image#request-parameters), [OpenRouter image prices](https://openrouter.ai/api/v1/images/models/microsoft/mai-image-2.6/endpoints), and [OpenRouter token ceilings](https://openrouter.ai/api/v1/models/microsoft/mai-image-2.6/endpoints).
