...
- 空目录、非法/重复 ID、空协议列表（包括新发现型号）、协议类型异常、HTTP/网络/JSON 错误或无效复核配置：退出 1，不声称检查成功。
- 缺失复核绑定精确 ID、角色、所选协议、日期、来源与理由；修改角色/协议必须重新审核。恢复的型号独立列出并触发 review/warning，即使恢复后元数据与原审核目录完全相同；运行时照常刷新并校验。审核恢复后应移除对应的缺失复核项，避免掩盖该型号未来再次缺失的新事件。
