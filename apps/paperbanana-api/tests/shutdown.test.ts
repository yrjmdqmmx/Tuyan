...
test('service composition configures legacy admission, cached health, and tracked shutdown hooks', () => {
  ...
  assert.match(source, /stopAdmission\(\) \{ legacyLifecycle\.stop\(\); figureOperations\.stop\(\) \}/)
  assert.match(source, /drainJobs\(\) \{ await Promise\.all\(\[legacyLifecycle\.drain\(\), figureOperations\.drain\(\)\]\)/)
