...
test('all catalogs keep deterministic chronology after refresh, without changing routes or IDs', async () => {
  ...
  assert.deepEqual(orderModelChannels(ids), ['tokendance','siliconflow','deepseek','kimi','zhipu','bailian','ark','minimax','tokenhub','xiaomi','anthropic','recraft','xai','gemini','openai','bfl','stability','ideogram','mistral','together','fireworks','fal','replicate','runware'])
