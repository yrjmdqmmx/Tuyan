// Keep text redaction aligned with auth-gateway/src/redaction.js. DTOs still use explicit field allowlists.
export function redactAdminText(value: unknown) {
  return redactApiKeyMaps(String(value || ''))
    .replace(/(\b(?:cookie|set-cookie)\s*:\s*)[^\r\n]+/gi, '$1[REDACTED]')
    .replace(
      /(\\?"(?:authorization|cookie|password|token|secret|(?:verification[_-]?status|admin|gateway|guest|access|refresh|session|bearer)[_-]?token|api[_-]?key|access[_-]?key(?:id|secret))\\?"\s*:\s*\\?")((?:\\.|[^"\\])*)(\\?")/gi,
      '$1[REDACTED]$3',
    )
    .replace(
      /((?:['"])?(?:authorization|cookie|password|token|secret|(?:verification[_-]?status|admin|gateway|guest|access|refresh|session|bearer)[_-]?token|api[_-]?key|access[_-]?key(?:id|secret))(?:['"])?\s*:\s*')((?:\\.|[^'\\])*)(')/gi,
    ...
    .replace(
      /((?:authorization|cookie|api[_-]?key|access[_-]?key(?:id|secret)|(?:verification[_-]?status|admin|gateway|guest|access|refresh|session|bearer)[_-]?token|token|secret|password)\s*[:=]\s*)(?:Bearer\s+)?[^\s,;&}"']+/gi,
      '$1[REDACTED]',
    );
}

function redactApiKeyMaps(value: string) {
...
function serializedValueEnd(value: string, valueStart: number) {
...
function matchingQuotedEnd(value: string, valueStart: number, quote: string) {
...
function matchingStructuredEnd(value: string, valueStart: number) {
  const stack = [];
  let quote = '';
