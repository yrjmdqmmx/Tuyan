...
function integer(env: Record<string, string | undefined>, name: string) {
  const raw = String(env[name] || '')
  if (!/^[1-9][0-9]{0,3}$/.test(raw)) throw new Error('BENCHMARK_PHASE_OPERATOR_AUTHORIZATION_INVALID')
  return Number(raw)
}
...
export function parseBenchmarkPhaseAuthorization(env: Record<string, string | undefined>) {
  ...
  const expectedState = phase === 'quick' ? 'quick_running' : phase === 'full' ? 'full_running' : ''
  ...
  const lane = String(env.PAPERBANANA_BENCH_PHASE_OPERATOR_LANE || '') as BenchmarkLane
  ...
  const maxJudgments = integer(env, 'PAPERBANANA_BENCH_MAX_JUDGMENTS')
  const maxJudgeCalls = integer(env, 'PAPERBANANA_BENCH_MAX_JUDGE_CALLS')
  ...
  const estimatedPerJudgeCall = amount(env, 'PAPERBANANA_BENCH_ESTIMATED_PER_JUDGE_CALL_USD')
  ...
  const generationCap = phase === 'quick' ? 24 : phase === 'full' ? 144 : 0
  const judgmentCap = phase === 'quick' ? 48 : phase === 'full' ? 288 : 0
  const judgeCallCap = judgmentCap * 4
  const usdCap = phase === 'quick' ? 100 : phase === 'full' ? 1_000 : 0
  ...
  const authorizationBase = {
    schemaVersion: 1 as const, phase, expectedState: expectedState as 'quick_running' | 'full_running', runId,
