import { canonicalHash, scientificMaxAttempts } from '@paperbanana/benchmark-core'
...
export function reconcileScientificV2UnknownNoArtifact(
  ...
  if (!attempt || attempt.responseClass !== 'unknown_provider_outcome' || attempt.actualCny !== null
    ...
    || attempt.height !== null || attempt.format !== null || attempt.attemptIndex > scientificMaxAttempts(slot.provider)) fail()
  ...
  slot.status = attempt.attemptIndex === scientificMaxAttempts(slot.provider) ? 'failed' : 'retrying'
