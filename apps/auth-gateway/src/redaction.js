export function redactText(value) {
  return redactApiKeyMaps(String(value || ''))
    ...
    .replace(
      /(\\?"(?:authorization|cookie|password|code|state|code[_-]?verifier|client[_-]?secret|confirmation[_-]?token|token|secret|(?:verification[_-]?status|confirmation|admin|gateway|guest|access|refresh|session|bearer)[_-]?token|api[_-]?key|access[_-]?key(?:id|secret))\\?"\s*:\s*\\?")((?:\\.|[^"\\])*)(\\?")/gi,
      '$1[REDACTED]$3',
    )
    .replace(
      /((?:['"])?(?:authorization|cookie|password|code|state|code[_-]?verifier|client[_-]?secret|confirmation[_-]?token|token|secret|(?:verification[_-]?status|confirmation|admin|gateway|guest|access|refresh|session|bearer)[_-]?token|api[_-]?key|access[_-]?key(?:id|secret))(?:['"])?\s*:\s*')((?:\\.|[^'\\])*)(')/gi,
    ...
    .replace(/([?&](?:api[_-]?key|key|token|secret|code|state|code[_-]?verifier|client[_-]?secret|confirmation[_-]?token)=)[^&\s]+/gi, '$1[REDACTED]')
    .replace(
      /((?:authorization|cookie|api[_-]?key|access[_-]?key(?:id|secret)|(?:verification[_-]?status|confirmation|admin|gateway|guest|access|refresh|session|bearer)[_-]?token|token|secret|password|code|state|code[_-]?verifier|client[_-]?secret|confirmation[_-]?token)\s*[:=]\s*)(?:Bearer\s+)?[^\s,;&}"']+/gi,
      '$1[REDACTED]',
    );
}
