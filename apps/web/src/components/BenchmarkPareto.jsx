...
import { OFFICIAL_PRICES, filterModels, formatCost, groupPoints, metricScore, officialPrice, paretoFrontier } from './benchmarkParetoMath.js'
...
function brandIcon(model) {
  const file = model.modelId.startsWith('black-forest-labs/') ? 'bfl.png' : model.modelId.startsWith('qwen') ? 'qwen.png' : model.modelId.startsWith('wan') ? 'wan.ico' : /^(seedream|doubao)/.test(model.modelId) ? 'bytedance.png' : model.modelId.startsWith('recraft/') ? 'recraft.png' : model.modelId.startsWith('krea/') ? 'krea.png' : model.modelId.startsWith('x-ai/') ? 'xai.svg' : null
...
function Sources({ price }) { return <span className="pareto-sources">{price.sources.map((s, i) => <a href={s.url} target="_blank" rel="noreferrer" key={s.url}>{s.label}<ExternalLink size={11} />{i < price.sources.length - 1 ? ' ' : ''}</a>)}</span> }
...
function PriceDetail({ row, metric, label, onClose }) {
  ...
  return <div className="pareto-inspect" aria-label="模型计价详情">
    ...
    <div className="pareto-inspect-values"><span>{label}<b>{metricScore(model, metric)?.toFixed(2) ?? '—'}<small> / 10</small></b></span><span>标准成本<b>{formatCost(price.usd)}<small> / 张</small></b></span></div>
    <p>{price.conditions}</p><p>{price.rateText}</p>
...
function Scatter({ rows, frontier, metric, label, activeId, onActive }) {
  ...
  return <div className="pareto-chart">
    ...
    <div className="pareto-chart-scroll" tabIndex={0} aria-label="成本与评分散点图，可横向滚动">
      <svg className="pareto-svg" viewBox="0 0 790 470" role="group" aria-label={`${label}与官方定价折算成本，${rows.length} 个模型`} style={{ touchAction: zoom > 1 ? 'none' : 'auto' }}
        ...
        <text x="410" y="460" textAnchor="middle" className="pareto-axis-title">官方定价折算成本（USD/张） →</text>
        <g clipPath={`url(#${clip})`}>
          ...
          {groups.map(group => {
            ...
            const content = <><title>{group.map(r => r.model.displayName).join(' / ')} · {score.toFixed(2)} · {formatCost(row.price.usd)}{group.length > 1 ? '（同成本同分）' : ''}</title><circle r="22" fill="transparent" /><rect x="-14" y="-14" width="28" height="28" rx="8" fill="var(--bench-paper)" stroke={optimal ? '#41735f' : color} strokeWidth={active ? 3 : optimal ? 2 : 1} />{brandIcon(model) ? <image href={brandIcon(model)} x="-10" y="-10" width="20" height="20" aria-hidden="true" /> : <text textAnchor="middle" y="5" fill={color} fontWeight="800" fontSize={mark.length > 1 ? 9 : 15}>{mark}</text>}{group.length > 1 && <><circle cx="13" cy="-13" r="8" fill="#191b1f" /><text x="13" y="-10" textAnchor="middle" fontSize="9" fill="white">{group.length}</text></>}{active && <circle r="20" fill="none" stroke="#e9aa21" strokeWidth="2" />}</>
            return <g key={model.modelId} transform={`translate(${x(row.price.usd)},${y(score)})`} data-point="true" className={`pareto-point${optimal ? ' is-frontier' : ''}`}>
    ...
    <div className="pareto-legend"><span><i />帕累托最优</span><span><i />其他可比模型</span><small>悬浮或键盘聚焦查看计价；点击模型进入详情。同坐标模型合并显示。</small></div>
  </div>
}

function PriceTable({ rows }) {
  return <div className="pareto-price-table-wrap" tabIndex={0} aria-label="逐模型官方价格来源表，可横向滚动"><table className="pareto-price-table"><thead><tr><th>模型 / 准确 ID</th><th>标准成本 · USD/张</th><th>官方 API 定价 / 条件</th><th>价格来源 / 查询日期</th></tr></thead><tbody>{rows.map(({ model, price }) => <tr key={model.modelId}>
...
export default function BenchmarkPareto({ models, axes }) {
  ...
  const rows = useMemo(() => models.map(model => ({ model, price: officialPrice(model) })), [models])
  ...
  return <section className="pareto-view" aria-label="帕累托视图" onKeyDown={e => { if (e.key === 'Escape') setActiveIds([]) }}>
    <div className="pareto-intro"><div><span className="bench-eyebrow">QUALITY × COST</span><h2>为科研图示，找到更合适的模型</h2><p>把当前评测分数与官方 API 标准价格放在一起比较。</p></div><a href="#pareto-pricing">定价依据与计算说明 <ArrowDown size={14} /></a></div>
    <div className="pareto-workspace">
      <aside className="pareto-filters" aria-label="帕累托筛选"><h3>比较条件</h3>
        ...
        <div className="pareto-coverage"><span><b>{comparable}</b> / {models.length}</span><p>模型具备完整可比成本</p><small>{models.length - comparable} 个模型暂不绘入成本图，仍保留全部正式评分与普通排名。</small></div>
        <div className="pareto-price-note"><strong>标准价格折算值</strong><p>6 道生成 + 3 道编辑，9 题等权，每题 1 张。按各模型原测试条件匹配当前标准价格。</p><small>分辨率随原评测条件而异；不是统一分辨率重测。排除折扣、账户优惠和历史实际费用。</small></div>
      </aside>
      <div className="pareto-main"><div className="pareto-scope" role="status" aria-label="比较范围"><span>当前筛选 · {label}</span><strong>{filtered.visible.length} 个可比模型 · {frontier.length} 个前沿模型</strong></div><div className="pareto-plot-layout">
        <Scatter key={`${metric}|${query}|${vendor}|${min}|${max}`} rows={filtered.visible} frontier={frontier} metric={metric} label={label} activeId={active[0]?.model.modelId} onActive={select} />
        <aside className="pareto-frontier" aria-label="帕累托最优模型"><header><h3>帕累托最优模型</h3><span>{frontier.length}</span></header><p>仅在当前维度和筛选范围内计算</p><ol>{[...frontier].reverse().map(row => <li key={row.model.modelId} className={activeIds.includes(row.model.modelId) ? 'is-active' : ''}><button aria-label={`在图中高亮 ${row.model.displayName}`} onMouseEnter={() => select([row])} onFocus={() => select([row])} onClick={() => select([row])}><Brand model={row.model} /><span><strong>{row.model.displayName}</strong><small>{row.model.developer}</small></span><span className="pareto-frontier-value"><b>{metricScore(row.model, metric).toFixed(2)}</b><small>{formatCost(row.price.usd)}/张</small></span></button><a href={profileHref(row.model)} aria-label={`查看 ${row.model.displayName} 详情`}><ArrowRight size={15} /></a></li>)}</ol>{frontier.length === 0 && <p>暂无可比较的前沿模型。</p>}
    ...
    <section className="pareto-methodology" id="pareto-pricing"><div className="bench-eyebrow">PRICING NOTES</div><h2>每一笔标准成本，都有依据</h2><div className="pareto-method-grid"><div><b>01 / 同一题集，同一权重</b><p>标准成本 =（6 道生成报价 + 3 道编辑报价）÷ 9。每道编辑题含 1 张 2048×1152 源图。使用原产物像素、请求分辨率和质量条件，生成题无参考图；不重测、不修改评测分数。</p></div><div><b>02 / 计价完整才进入图表</b><p>按 token、像素或运行量计费时须有可核验的计费量。失败仍保留原评分；缺失产物不等于免费。这里比较九题标准报价，不累计历史重试费用，也不按成功图片数重新加权。</p></div><div><b>03 / 原币价格，统一折算</b><p>CNY 价格按 ECB {OFFICIAL_PRICES.fx.date} 参考汇率换算：CNY × 1.1460 ÷ 7.6755 = USD。USD 价格不换汇。无优惠、套餐、批量折扣或实际扣费参与。</p><a href={OFFICIAL_PRICES.fx.source} target="_blank" rel="noreferrer">ECB 参考汇率 <ExternalLink size={12} /></a></div></div><p className="pareto-method-footnote">查询日期：{OFFICIAL_PRICES.checkedAt}。成本与分数均以未四舍五入值判断支配关系；同成本同分的模型同时保留。价格快照与历史测试费用独立，官方当前别名不等同于可追溯的底层权重快照。</p><details className="pareto-source-details"><summary>逐模型官方价格来源表 · 当前正式榜单 {rows.length} 个模型</summary><PriceTable rows={rows} /></details></section>
  </section>
}
