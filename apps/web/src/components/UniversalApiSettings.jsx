...
function ConnectionOptions({draft, label, onChange}) {
  ...
  return <details className="universal-details">
    <summary>接入地址与兼容选项</summary>
    <div className="universal-detail-body">
      <label className="field"><span>Base URL（含版本，不含操作路径）</span><input aria-label={`${label} Base URL`} value={c.baseUrl} onChange={e=>custom({baseUrl:e.target.value})} spellCheck={false}/><small>官方地址已预填。第三方服务请按文档填写，例如 https://example.com/v1，不含 /chat/completions 等操作路径。</small></label>
      {c.protocol==='dashscope-multimodal'&&<p className="universal-hint">百炼新业务空间需填写对应地域的 Workspace 专属地址；默认值是旧版北京地址。</p>}
      ...
      {['openai-chat','openai-images'].includes(c.protocol)&&<label className="field"><span>调用兼容变体</span><select aria-label={`${label} 兼容变体`} value={c.compatibility} onChange={e=>custom({compatibility:e.target.value})}><option value="standard">标准协议</option>{c.protocol==='openai-chat'?<option value="openrouter-image">OpenRouter 图片输出扩展</option>:<option value="ark-images">Ark JSON 图片输入</option>}</select></label>}
      <label className="field"><span>模型目录接口规则</span><select aria-label={`${label} 目录接口规则`} value={c.catalogFormat||'auto'} onChange={e=>custom({catalogFormat:e.target.value})}>{catalogFormats.map(([id,title])=><option key={id} value={id}>{title}</option>)}</select><small>未知服务默认不探测接口。仅当服务文档明确兼容所选目录格式及 /models 路径时，才选择对应规则；否则手动填写 ID。</small></label>
    </div>
  </details>
}

function UniversalRole({role, draft, credential, onChange, onKeyChange, onCopy, apiBase, health, contractSupported}) {
  const [label,description] = roleLabels[role], c = draft.custom
  ...
  return <fieldset className="universal-role" aria-label={label}>
    <legend>{label}</legend>
    <div className="universal-role-heading"><p>{description}</p>{role!=='main'&&<button type="button" className="universal-button universal-copy" onClick={()=>{setFeedbackReady(false);onCopy()}}><Copy size={14}/>复制主模型接入与密钥</button>}</div>
    ...
    <div className="universal-catalog-toolbar"><button type="button" className="universal-button" disabled={Boolean(busy)||!contractSupported||!strategy.supported||!credential?.apiKey?.trim()} onClick={()=>check('catalog')}>{busy==='catalog'?<Loader2 className="universal-spinner" size={16}/>:<Download size={16}/>} {busy==='catalog'?'正在获取模型…':visibleCatalog&&!visibleCatalog.error?'重新获取模型':'获取模型'}</button><span>只读目录 · 不触发生成</span></div>
    <p className="universal-hint">{!strategy.supported?strategy.message:!credential?.apiKey?.trim()?'填写当前服务的 API Key 后可获取模型，无需先填模型 ID。':strategy.message}</p>
    {visibleCatalog&&<div className={`universal-status ${visibleCatalog.error?'error':visibleCatalog.warnings?.length?'warning':'neutral'}`} role={visibleCatalog.error?'alert':'status'}>
      ...
      {!visibleCatalog.error&&<small>本次获取 {visibleCatalog.models.length} 个有效 ID{visibleCatalog.warnings?.length?`，已隔离 ${visibleCatalog.warnings.length} 条异常`:''}。{visibleCatalog.fetchedAt?`读取时间：${new Date(visibleCatalog.fetchedAt).toLocaleTimeString('zh-CN')}。`:''}目录可见不代表能力、权限或真实调用验证成功。</small>}
      {selectedMissing&&<p>当前 ID「{draft.modelId}」未出现在本次目录中，已保留。可核对文档后继续手动配置；目录可能不完整。</p>}
    </div>}
    {visibleCatalog?.models.length>0&&<div className="universal-catalog-picker">
      <label className="universal-search"><Search size={16}/><input aria-label={`${label} 搜索模型`} placeholder="搜索准确模型 ID" value={query} onChange={e=>setQuery(e.target.value)}/></label>
      <div className="universal-model-list" role="group" aria-label={`${label} 目录模型`}>
        {matches.map(({id})=><button type="button" key={id} aria-pressed={id===draft.modelId} className={id===draft.modelId?'selected':''} onClick={()=>{onChange({modelId:id});setFeedbackReady(true);setEditing(false)}}><span>{id}</span>{id===draft.modelId?<Check size={16}/>:<small>选择</small>}</button>)}
        {!matches.length&&<p className="universal-hint">没有匹配项，可修改搜索或手动填写。</p>}
      </div>
    </div>}
    <label className="field"><span>准确模型 ID <small>支持手动填写</small></span><input aria-label={`${label} 模型 ID`} value={draft.modelId} autoComplete="off" spellCheck={false} onFocus={()=>setEditing(true)} onBlur={()=>{setEditing(false);setFeedbackReady(true)}} onChange={e=>change({modelId:e.target.value})} placeholder="从目录选择，或粘贴服务提供的准确 ID"/></label>
    {!draft.modelId.trim()?<p className="universal-hint">{feedback.message}</p>:feedbackReady&&!editing&&<p className={`universal-status ${feedback.tone}`} role={feedback.tone==='error'?'alert':'status'}>{feedback.message}</p>}
    <details className="universal-details"><summary>能力与限额</summary><UniversalCapabilities draft={draft} label={label} onChange={change}/></details>
    <button type="button" className="universal-button" disabled={Boolean(busy)||!contractSupported} onClick={()=>check('config')}>{busy==='config'?<Loader2 className="universal-spinner" size={16}/>:<ShieldCheck size={16}/>}检查配置与地址</button>
...
export default function UniversalApiSettings({drafts,keys,onChange,onKeyChange,onCopy,onSave,apiBase,health,contractSupported, roles = ['main','vision','image']}) {
  const [saved,setSaved]=useState('')
  return <section className="universal-api-settings" aria-label="通用 API 接入">
    <p className="universal-intro">接入地址决定服务渠道，API 协议决定请求格式，模型 ID 决定实际型号。各角色可复用接入信息。</p>
    {!contractSupported&&<p className="universal-status error" role="alert">当前后端暂不支持通用 API，配置已保留，请稍后重试。</p>}
    <details className="universal-privacy"><summary><ShieldCheck size={15}/>密钥与验证说明</summary><p>密钥仅留在当前页面；恢复任务所需密钥在服务端加密保存，完成后删除，最长 7 天。地址或协议改变后需重新填密钥。保存配置不会保存密钥。</p><p>配置校验、目录获取、真实调用是三个不同状态。本页检查不会触发付费生成。</p></details>
    {roles.filter(role=>Object.hasOwn(roleLabels,role)).map(role=><UniversalRole key={role} role={role} draft={drafts[role]} credential={keys[role]} onChange={patch=>{onChange(role,patch);setSaved('')}} onKeyChange={value=>onKeyChange(role,value)} onCopy={()=>onCopy(role,'main')} apiBase={apiBase} health={health} contractSupported={contractSupported}/>)}
    <button type="button" className="universal-button universal-save" onClick={()=>setSaved(onSave()?'配置已保存，未保存密钥或验证状态。':'浏览器存储不可用，配置仍保留在当前页面。')}>保存非敏感配置</button>{saved&&<p className="universal-status neutral" role="status">{saved}</p>}
  </section>
}
