...
export default function ContactDialog({ open, onClose }) {
  ...
  return <div className="feedback-dialog-backdrop" onClick={onClose}>
    <section className="contact-dialog" role="dialog" aria-modal="true" aria-labelledby="contact-dialog-title" onClick={event => event.stopPropagation()}>
      <button ref={closeRef} type="button" className="contact-dialog-close" aria-label={t("关闭")} onClick={onClose}><X size={18} /></button>
      <h2 id="contact-dialog-title">{t("联系作者")}</h2>
      <p>{t("使用中有任何问题、建议或合作意向，欢迎扫码添加作者微信。")}</p>
      {qrFailed ? <div className="contact-qr-fallback">{t("二维码即将上线，可先点顶栏「意见反馈」联系作者。")}</div>
        : <img className="contact-qr" src={appPath('/contact-qr.png')} alt={t("作者微信二维码（赵）")} onError={() => setQrFailed(true)} />}
    </section>
  </div>;
}
