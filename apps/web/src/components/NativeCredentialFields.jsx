  ...
  return <div className="credential-provider">
    {provider === 'minimax' ? <label className="field">
      ...
      <select aria-label="MiniMax 区域" value={minimaxRegion(providerRegions)} onChange={(event) => onMiniMaxRegionChange(event.target.value)}>
        {Object.entries(MINIMAX_REGIONS).map(([id, entry]) => <option key={id} value={id} disabled={id === 'cn' && !regionContractSupported}>{entry.label}</option>)}
      </select>
      <small>{region.apiBase} · 请填写该区域平台的 Key</small>
    </label> : null}
    <label className="field">
      <span>{label} 接入密钥</span>
      <div className="key-input">
        <KeyRound size={18} />
        <input type="password" aria-label={`${label} 接入密钥`} value={value || ''} onChange={(event) => onChange(event.target.value)} placeholder={providerConfig.keyPlaceholder} autoComplete="off" />
      </div>
    </label>
    <ApiKeyGuide recoverable={recoverable} providerConfig={provider === 'minimax'
      ? { ...providerConfig, guideUrl: region.keyUrl, guideSteps: ['登录所选区域的 MiniMax 开放平台并创建 API Key。', '不同区域的 Key 分别保存在当前页面内存，切换时不会互用。'] }
      : providerConfig} />
  </div>;
}
