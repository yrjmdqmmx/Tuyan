      ...
      <section className="feedback-dialog" role="dialog" aria-modal="true" aria-labelledby="feedback-title">
        <div className="feedback-dialog-head">
          <div className="section-head">
            ...
            <div>
              <h2 id="feedback-title">{t("意见反馈")}</h2>
              <p>{t("告诉我们哪里需要修复或改进。")}</p>
            </div>
          </div>
          <button ref={closeButtonRef} type="button" className="feedback-close-button" onClick={onClose} aria-label={t("关闭意见反馈")} disabled={isSubmitting}>
            <X size={18} />
          </button>
        </div>
        ...
        <form className="feedback-form" onSubmit={submitFeedback}>
          ...
          <label className="field">
            ...
            <textarea
              ...
              maxLength={2000}
              rows={7}
              placeholder={t("描述遇到的问题、期望的功能或使用体验。")}
            />
          ...
          <label className="field">
            ...
            <input
              ...
              maxLength={300}
              placeholder={t("可选：邮箱、微信或其他联系方式")}
            />
          </label>

          <button className="primary-button" type="submit" disabled={!canSubmit}>
            {isSubmitting ? <Loader2 className="spin" size={18} /> : <Send size={18} />}{t("提交反馈")}</button>
          {success ? <div className="success-line"><CheckCircle2 size={16} />{t(" 反馈已提交，感谢你的意见。")}</div> : null}
