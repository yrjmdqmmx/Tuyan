...
export default function AgentConnectionDialog({ open, onClose }) {
  ...
  return (
    <AccessibleDialog open={open} onClose={close} labelledBy="agent-connection-title" describedBy="agent-connection-description" className="agent-connection-dialog" backdropClassName="agent-connection-backdrop">
      <button type="button" className="agent-connection-close" aria-label={t("关闭智能体接入")} data-autofocus onClick={close}><X size={18} /></button>
      <div className="agent-connection-heading">
        <span className="agent-connection-icon" aria-hidden="true"><Bot size={25} /></span>
        <h2 id="agent-connection-title">{t("连接你的智能体")}</h2>
      </div>
      <p id="agent-connection-description">{t("复制下方口令，发送给你的 Agent，按提示完成图研接入。")}</p>
      <label className="agent-prompt-label" htmlFor="agent-connection-prompt">{t("接入口令")}</label>
      ...
      <button type="button" className="agent-copy-button" onClick={copyPrompt} disabled={copyState === 'copying'}>
        ...
        {t(copyState === 'copying' ? '复制中…' : copyState === 'copied' ? '口令已复制' : '复制接入口令')}
      </button>
      <div className="agent-copy-feedback" role="status" aria-live="polite" aria-atomic="true">
        {t(copyState === 'copied' && '口令已复制，请发送给你的 Agent。配置和验证由 Agent 继续完成。')}
        {copyState === 'failed' && <><span>{t("复制失败，已选中口令，请手动复制。")}</span><button type="button" className="text-button" onClick={selectPrompt}>{t("选择全部口令")}</button></>}
        {t(copyState === 'copying' && '正在复制口令…')}
      </div>
      <div className="agent-capabilities">
        <section><h3>{t("Skill · 本地科研图示工作流")}</h3><p>{t("组织图示规划、评审与精修，使用 Agent 自有的生图或代码工具；没有生图能力时，可先完成图示规格。")}</p></section>
        <section><h3>{t("MCP · 只读公共知识")}</h3><p>{t("读取图研模板、规则与 Schema。")}</p></section>
      </div>
      <nav className="agent-document-links" aria-label={t("手动接入文档")}>
        <a href={AGENT_INSTALLATION_URL} target="_blank" rel="noopener noreferrer">{t("官方接入说明 ")}<ExternalLink size={13} /></a>
        <a href={AGENT_SKILL_URL} target="_blank" rel="noopener noreferrer">{t("Skill 文档 ")}<ExternalLink size={13} /></a>
        <a href={AGENT_MCP_URL} target="_blank" rel="noopener noreferrer">{t("MCP 文档 ")}<ExternalLink size={13} /></a>
      </nav>
    </AccessibleDialog>
  );
}
