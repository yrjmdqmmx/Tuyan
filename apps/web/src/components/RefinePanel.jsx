        ...
        <section className="refine-step" aria-labelledby="refine-source-title">
          ...
          <div className={`refine-dropzone${dragging ? ' dragging' : ''}${source.url ? ' has-source' : ''}`}
            ...
            {source.url ? <figure className="refine-original"><img src={source.url} alt="待精修原图" /><figcaption>{source.filename || '来自任务结果'}{source.width ? ` · ${source.width} × ${source.height}` : ''}</figcaption></figure>
              : <button className="refine-upload-button" type="button" disabled={!allowUpload} onClick={chooseFile}><Upload size={26} /><strong>点击上传或拖拽图片到这里</strong><span>{uploadEnabled ? `PNG / JPG / WebP · 最大 ${(uploadLimits.maxBytes / 1024 / 1024).toFixed(1)} MiB` : '连接支持上传精修的后端后即可上传'}</span></button>}
            {dragging ? <div className="refine-drop-overlay">松开以上传原图</div> : null}
          </div>
          {uploadLimits?.processingHint ? <details className="reference-processing-details"><summary>模型限制与图片处理</summary><p className="reference-processing-note">{uploadLimits.processingHint}</p></details> : null}
