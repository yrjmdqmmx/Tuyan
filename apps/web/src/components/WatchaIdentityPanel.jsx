...
export default function WatchaIdentityPanel({ controller, user, onSignIn }) {
  ...
  return <section className="watcha-identity" aria-label="观猹身份登录">
    <div className="watcha-heading"><WatchaMark /><strong>{linked ? '已绑定观猹' : pending ? '观猹授权已完成' : '观猹登录'}</strong></div>
    {!c.status.available ? <p>观猹登录状态暂时无法获取，可继续使用邮箱登录。</p> : linked ? <><p>使用观猹登录同一个图研账号，继续访问已有任务和图片。</p>
      ...
      {user ? <><p>将观猹绑定到当前图研账号。你的已有任务和渠道连接保持归属不变。</p>
        {c.status.emailVerified ? <button className="account-button" type="button" disabled={c.busy} onClick={c.link}><WatchaMark />确认绑定当前图研账号</button> : <><p>请先验证当前图研邮箱，然后确认绑定。</p>{verifyEmailButton}</>}
      </> : <>
    ...
    </> : user && !c.status.emailVerified ? <><p>绑定观猹前，请先验证当前图研邮箱。</p>{verifyEmailButton}</> : <><p>{user ? '绑定后可使用观猹登录当前图研账号。' : '使用观猹账号登录，保留当前工作台内容。'}</p><button className="account-button watcha-login-button" type="button" disabled={c.busy} onClick={() => c.start(user ? 'link' : 'login')}><WatchaMark />{c.busy ? '等待完成授权…' : user ? '绑定观猹账号' : '使用观猹登录'}</button></>}
