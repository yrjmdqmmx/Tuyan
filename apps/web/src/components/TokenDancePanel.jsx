...
export function TokenDanceRecovery({ job, controller: td, onResumed, onOpenAccount, customKeys }) {
  ...
  return <section className="tokendance-panel td-recovery" aria-label="任务恢复"><strong>任务及成功步骤已保存</strong><p>{recovery.message}</p>{recovery.channel === 'custom' && <p>请先在服务商处处理余额或权限。若需更换密钥，在通用 API 设置中填写同一地址的新密钥，再继续原任务；恢复不会更改原模型与协议。</p>}
    <div className="td-actions">{(!recovery.channel || recovery.channel === 'tokendance') && <button type="button" className="account-button" onClick={onOpenAccount}>前往账户处理</button>}
    {recovery.canResume && <button type="button" className="account-button account-button-primary" disabled={td.busy || (recovery.retryAt && Date.parse(recovery.retryAt) > Math.max(now, Date.now()))} onClick={() => td.perform(async () => { const result = await td.request((!recovery.channel || recovery.channel === 'tokendance') ? 'tokenDanceResume' : 'providerResume', { jobId: job.id, ...(recovery.channel === 'custom' && customKeys ? {apiKeys:{custom:customKeys}} : {}) }); await onResumed(result.jobId); })}>从已完成步骤继续</button>}</div>
