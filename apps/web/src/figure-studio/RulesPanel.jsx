...
export default function RulesPanel({ document, onCommands, readOnly = false }) {
  ...
  return <div className="fs-rules fs-panel-content">
    ...
    {scope === 'working' && !readOnly && <details className="fs-preset-tools"><summary>本机个人预设</summary><label className="fs-field"><span>保存当前规则为</span><input value={presetName} maxLength={60} onChange={(event) => setPresetName(event.target.value)} placeholder="例如：课题组投稿检查" /></label><button className="fs-wide" disabled={!presetName.trim()} onClick={savePreset}>保存当前工作规则</button>{presets.length > 0 && <><label className="fs-field"><span>已保存预设</span><select value={selectedPreset} onChange={(event) => setSelectedPreset(event.target.value)}><option value="">选择本机预设</option>{presets.map((preset) => <option key={preset.id} value={preset.id}>{preset.name}</option>)}</select></label><div className="fs-button-row"><button disabled={!selectedPreset} onClick={applyPreset}>应用预设</button><button disabled={!selectedPreset} onClick={deletePreset}>删除预设</button></div></>}{presetMessage && <p role="status" className="fs-micro">{presetMessage}</p>}<p className="fs-micro">仅保存规则，不保存研究材料、画布内容或密钥。同名保存会更新已有预设。</p></details>}
    ...
    })}<div className="fs-custom-rule"><h4>添加工作规则</h4><label className="fs-field"><span>名称</span><input value={customName} maxLength={120} onChange={(event) => setCustomName(event.target.value)} placeholder="例如：位图最低分辨率" /></label><label className="fs-field"><span>检查方式</span><select value={customKind} onChange={(event) => setCustomKind(event.target.value)}><option value="min-dpi">最低图片 DPI</option><option value="manual">人工确认项</option></select></label>{customKind === 'min-dpi' ? <Field label="最低 DPI" type="number" min={1} max={2400} value={customValue} onCommit={setCustomValue} /> : <label className="fs-field"><span>确认说明</span><textarea value={customMessage} rows={3} maxLength={1000} onChange={(event) => setCustomMessage(event.target.value)} placeholder="记录需要作者核对的具体事项" /></label>}<button className="fs-wide" disabled={!customName.trim()} onClick={addRule}>添加此工作规则</button></div></div>}
