...
import TokenDancePanel, { TokenDanceStatus } from '../components/TokenDancePanel.jsx';
import { useTokenDance } from '../hooks/useTokenDance.js';
import { bindUniversalKey, loadUniversalDrafts, updateUniversalDraft } from '../lib/universalApi.js';
...
import { saveStudioUniversalDraft, studioModelRegistry, studioModelSelection, studioProviderSupported } from './modelSettings.js';
...
export function ModelSettings({ value, onChange, capabilities, userId, authReady = false }) {
  ...
  useEffect(() => {
    if (value.provider === 'tokendance') onChange(previous => previous.connected === tokenDance.connection.connected ? previous : { ...previous, connected: tokenDance.connection.connected });
  }, [value.provider, tokenDance.connection.connected, onChange]);
  useEffect(() => {
    if (nativeProvider) keyRing.current[regionApiKeySlot(value.provider, value.providerRegions)] = value.key || '';
  ...
  function useCustom() {
    if (!customMode) presetRoute.current = value;
    onChange(previous => ({ ...previous, provider: 'custom', modelId: customDraft.modelId, key: '', customDraft, customKey, valid: false }));
  }
  function usePreset() {
    if (!customMode) return;
    onChange(previous => ({ ...previous, ...(presetRoute.current || { provider: '', modelId: '', key: '' }), customDraft: undefined, customKey: undefined, valid: false }));
  }
  function changeCustom(_role, patch) {
    const next = updateUniversalDraft(customDraft, patch);
    setCustomDraft(next.draft);
    if (next.clearKey) setCustomKey(undefined);
    onChange(previous => ({ ...previous, modelId: next.draft.modelId, customDraft: next.draft, customKey: next.clearKey ? undefined : customKey, valid: false }));
  }
  function changeCustomKey(_role, key) {
    const bound = bindUniversalKey(customDraft, key); setCustomKey(bound);
    onChange(previous => ({ ...previous, customKey: bound }));
  }
  ...
  return <div className="fs-model-settings">
    ...
    <GenerationSettingsDrawer open={open} onClose={() => setOpen(false)}>
      ...
      <div className="field"><span>接入方式</span><div className="mode-switch" role="group" aria-label="画布模型接入方式">
        <button type="button" aria-pressed={!customMode} className={!customMode ? 'active' : ''} onClick={usePreset}><span>预设渠道</span><small>原生渠道与观猹</small></button>
        <button type="button" aria-pressed={customMode} className={customMode ? 'active' : ''} disabled={!customSupported && !customMode} onClick={useCustom}><KeyRound size={16} /><span>通用 API</span><small>自有服务 · 主模型</small></button>
      </div></div>
      {customMode ? <UniversalApiSettings roles={['main']} drafts={{ main: customDraft }} keys={{ main: customKey }} onChange={changeCustom} onKeyChange={changeCustomKey}
        onSave={() => saveStudioUniversalDraft(customDraft)} apiBase={API_BASE_DEFAULT} contractSupported={customSupported && registry?.universalApiContractVersion >= 1} /> : <ModelPicker label="主模型" role="main" route={{ accessProvider: value.provider, modelId: value.modelId }}
      ...
      {!customMode && <details className="api-keys-panel access-credentials" data-focus-setting="api-key" open>
        <summary><KeyRound size={17} /> 接入凭据</summary>
        <p>浏览器中的密钥仅保留在当前页面，退出账号或刷新后清除。AI 操作恢复所需输入与密钥会在服务端加密暂存，最长 7 天。</p>
        {nativeProvider ? <NativeCredentialFields provider={value.provider} providerConfig={PROVIDERS[value.provider]}
          ...
          regionContractSupported={Boolean(registry?.providerRegionContractVersion)} recoverable /> : value.provider === 'tokendance' ? <><TokenDanceStatus controller={tokenDance} onOpenAccount={() => setWalletOpen(!walletOpen)} />{walletOpen && <TokenDancePanel controller={tokenDance} />}</> : <p className="credential-empty">请明确选择图稿支持的渠道与主模型。</p>}
      </details>}
      ...
      <p className="fs-micro">目录、配置校验与真实调用是不同状态。仅开放当前图稿服务确认支持的接入方式；发起模型请求可能产生渠道费用。</p>
...
export function PlanPanel({ materials, setMaterials, plan, setPlan, busy, onPlan, onConfirm, limits, pending }) {
  return <div className="fs-panel-content fs-plan-panel">
    ...
    {plan && <div className="fs-plan-review"><div className="fs-section-heading"><h3>确认方案</h3><span className="fs-step">02</span></div><p className="fs-muted">修改下面的标题与节点，再确认生成。科学含义与关系由你把关。</p>
      ...
      {plan.nodes?.length > 1 && plan.edges?.length < 24 && <button onClick={() => setPlan({ ...plan, edges: [...plan.edges, { from: plan.nodes[0].id, to: plan.nodes[1].id }] })}>添加连接</button>}
...
export function EditPanel({ instruction, setInstruction, selected, scope, busy, onEdit, patch, onApply, onDismiss, revision, limits, pending }) {
  return <div className="fs-panel-content"><h3>用语言修改图稿</h3><p className="fs-muted">{selected ? `本次针对所选对象：${selected.text?.slice(0, 30) || selected.type}。` : '请先在画布中选择一个已有对象。'}修改会进入同一份图稿与撤销历史。新增、删除与分组请用画布工具。</p>
    ...
    <button className="primary-button fs-primary fs-wide" disabled={busy || pending || !instruction.trim() || !selected || !!scope?.error} onClick={onEdit}>{busy ? <Loader2 size={15} className="fs-spin" /> : <Send size={15} />}{busy ? '正在提出修改…' : '提出修改方案'}</button>
