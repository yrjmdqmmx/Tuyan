...
export function normalizeUniversalConnection(value) {
    if (!universalRecord(value) || value.version !== 1 || !universalString(value.connectionId, 80) || !/^[a-zA-Z0-9_-]+$/.test(value.connectionId) || !UNIVERSAL_PROTOCOLS.includes(value.protocol))
        throw new UniversalApiError('CATALOG_CONFIG_INVALID');
    ...
    if (!['bearer', 'x-api-key', 'x-goog-api-key'].includes(auth) || !['auto', 'openai', 'anthropic', 'gemini', 'none'].includes(catalogFormat))
        throw new UniversalApiError('CATALOG_CONFIG_INVALID');
...
export function resolveUniversalCatalogStrategy(value) {
    const c = normalizeUniversalConnection(value);
    if (c.catalogFormat === 'none')
        return { supported: false, format: null, source: 'unsupported', message: '此连接使用手动填写模型 ID；不会请求模型目录。' };
    if (c.catalogFormat !== 'auto')
        return { supported: true, format: c.catalogFormat, source: 'explicit', message: `按您确认的 ${c.catalogFormat} 目录格式读取 Base URL 下的 /models；目录可见不代表模型能力或权限已验证。` };
    let format = null;
    if (c.baseUrl === 'https://api.openai.com/v1' && c.protocol.startsWith('openai-') && c.auth === 'bearer')
        format = 'openai';
    if (c.baseUrl === 'https://openrouter.ai/api/v1' && c.protocol.startsWith('openai-') && c.auth === 'bearer')
        format = 'openai';
    if (c.baseUrl === 'https://api.anthropic.com/v1' && c.protocol === 'anthropic-messages' && c.auth === 'x-api-key')
        format = 'anthropic';
    if (c.baseUrl === 'https://generativelanguage.googleapis.com/v1beta' && c.protocol.startsWith('gemini-') && c.auth === 'x-goog-api-key')
        format = 'gemini';
...
export function universalCatalogError(error) {
    ...
    const known = universalRecord(error) && error.name === 'UniversalApiError' && typeof error.code === 'string' && Object.prototype.hasOwnProperty.call(universalErrorMessages, error.code)
        ? new UniversalApiError(error.code, 'not_sent', Number.isInteger(error.status) && error.status >= 400 && error.status <= 599 ? error.status : undefined) : undefined;
    if (known && (known.code.startsWith('CATALOG_') || ['ENDPOINT_UNSAFE', 'CREDENTIAL_MISMATCH'].includes(known.code)))
        return known;
    const code = known?.code === 'CONFIG_INVALID' ? 'CATALOG_CONFIG_INVALID' : known?.status === 401 ? 'CATALOG_AUTH_FAILED' : known?.status === 403 ? 'CATALOG_PERMISSION_DENIED'
...
export function universalConnectionCredential(connectionValue, serializedCustomKeys) {
