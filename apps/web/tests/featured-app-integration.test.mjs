...
test('initial sample is clean, then edited text requires confirmation and cancel preserves exact input', async () => {
  ...
  assert.equal(screen.getByLabelText(/论文方法内容/u).value, FEATURED_TEMPLATES[2].methodContent)
  assert.equal(screen.getByLabelText(/目标图注/u).value, FEATURED_TEMPLATES[2].caption)
  assert.equal(screen.getByLabelText(/负向提示词（可选）/u).value, FEATURED_TEMPLATES[2].negativePrompt)
})
