...
test('mobile More keeps all secondary actions, closes on selection, restores focus and responds to desktop resize', async () => {
  ...
  try {
    ...
    assert.deepEqual([...primary.querySelectorAll('.header-primary-link')].map(link => link.textContent.trim()), ['工作台','排行榜','更新日志']);
    ...
    assert.deepEqual([...dialog.getByRole('group',{name:'页面导航'}).querySelectorAll('a')].map(link=>link.textContent.trim()), ['工作台','排行榜','openacad','更新日志']);
