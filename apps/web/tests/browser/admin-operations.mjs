...
try {
 ...
 const firstRow=desktop.locator('.ops-jobs-table tbody tr').first();
 for(const text of ['Web 网页 · 专业模式','方法框架图','SVG 矢量图 · 检索：不检索','阶段：4 · 参考图：1']) assert.ok((await firstRow.innerText()).includes(text));
 let detailReads=0;desktop.on('request',request=>{if(request.url().endsWith('/paperbanana-api')&&request.postDataJSON()?.action==='adminTaskDetail')detailReads++});
 await firstRow.getByRole('button',{name:'展开任务内容',exact:true}).click();
 const preview=desktop.locator('.ops-task-preview');await preview.getByAltText('结果图 1',{exact:true}).waitFor();assert.equal(detailReads,1);
 const fact=(scope,name)=>scope.locator('.ops-facts dt').filter({hasText:new RegExp('^'+name+'$')}).locator('..').locator('dd');
 assert.equal(await fact(preview,'参考图识别模型').innerText(),'未使用');assert.equal(await fact(preview,'参考图处理').innerText(),'主模型直读');assert.equal(await fact(preview,'评审模式').innerText(),'文本评审');assert.equal(await fact(preview,'最多评审轮数').innerText(),'0');
 assert.match(await preview.innerText(),/收集样本 → 建立模型/);assert.match(await preview.innerText(),/图 1：实验流程/);assert.ok(!(await preview.innerText()).includes('CANARY'));
 checks.push('list restores saved configuration; one lazy detail request expands input and actual model usage');
 const singleDownload=desktop.waitForEvent('download');await preview.getByRole('button',{name:'下载结果图 1',exact:true}).click();const single=await singleDownload;assert.equal(single.suggestedFilename(),'paperbanana-result-1.svg');assert.match(fs.readFileSync(await single.path(),'utf8'),/<svg/);
 const refDownload=desktop.waitForEvent('download');await preview.getByRole('button',{name:'下载参考图 1',exact:true}).click();const reference=await refDownload;assert.equal(reference.suggestedFilename(),'paperbanana-reference-1.svg');assert.match(fs.readFileSync(await reference.path(),'utf8'),/<svg/);
 const zipDownload=desktop.waitForEvent('download');await preview.getByRole('button',{name:'下载全部',exact:true}).click();const zip=await zipDownload;assert.equal(zip.suggestedFilename(),'paperbanana-task-000.zip');const archive=await JSZip.loadAsync(fs.readFileSync(await zip.path()));
 for(const name of ['results/result-1.svg','references/reference-1.svg','stages/stage-01-generator.svg']) assert.match(await archive.file(name).async('string'),/<svg/);
 const metadata=await archive.file('metadata.json').async('string');assert.equal(JSON.parse(metadata).configurationMode,'advanced');assert.ok(!metadata.includes('CANARY'));checks.push('real result/reference downloads and ZIP contain image bytes plus redacted metadata');
 await preview.getByText('评审',{exact:true}).click();assert.equal(await fact(preview,'耗时').nth(2).innerText(),'0 毫秒');
 await desktop.locator('.ops-workspace').screenshot({path:out+'/00-task-expanded-desktop.png'});
 await preview.getByRole('button',{name:'进入详情与运营跟进 →',exact:true}).scrollIntoViewIfNeeded();const expandedScroll=await desktop.evaluate(()=>scrollY);await preview.getByRole('button',{name:'进入详情与运营跟进 →',exact:true}).click();await desktop.getByRole('heading',{name:'任务详情',exact:true}).waitFor();await desktop.getByRole('button',{name:'← 返回列表',exact:true}).click();await preview.getByAltText('结果图 1',{exact:true}).waitFor();await desktop.waitForFunction(y=>Math.abs(scrollY-y)<8,expandedScroll);assert.equal(await firstRow.getByRole('button',{name:'收起任务内容',exact:true}).getAttribute('aria-expanded'),'true');checks.push('expanded task and scroll position survive detail navigation');
 await firstRow.getByRole('button',{name:'收起任务内容',exact:true}).click();assert.equal(await preview.count(),0);
 ...
 await desktop.getByRole('button',{name:'肿瘤微环境机制图',exact:true}).click();await desktop.getByRole('heading',{name:'任务详情',exact:true}).waitFor();await desktop.getByAltText('结果图 1',{exact:true}).scrollIntoViewIfNeeded();await desktop.waitForFunction(()=>{const img=document.querySelector('img[alt="结果图 1"]');return img?.complete&&img.naturalWidth>0});
 ...
 await mobile.getByRole('button',{name:'展开任务内容',exact:true}).first().click();await mobile.locator('.ops-task-preview').getByAltText('结果图 1',{exact:true}).waitFor();assert.equal(await mobile.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await mobile.locator('.ops-task-preview').screenshot({path:out+'/07-task-expanded-mobile.png'});await mobile.getByRole('button',{name:'收起任务内容',exact:true}).click();checks.push('mobile inline preview fits viewport and keeps downloads accessible');
 await mobile.locator('.ops-table-wrap tbody tr').first().locator('td button').first().click();await mobile.getByRole('heading',{name:'任务详情',exact:true}).waitFor();await mobile.getByAltText('结果图 1',{exact:true}).waitFor();assert.equal(await mobile.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await mobile.locator('.ops-workspace').screenshot({path:out+'/06-task-detail-mobile.png'});checks.push('mobile detail and result');
 ...
 await desktop.goto(webBase+'/?admin=jobs&a_id=task-066');await desktop.getByRole('heading',{name:'任务详情',exact:true}).waitFor();await desktop.getByRole('heading',{name:'生成配置',exact:true}).waitFor();
 for(const field of ['模式','格式','检索','阶段','参考图处理','最多评审轮数']) assert.equal(await fact(desktop,field).innerText(),'未记录');
 await desktop.getByRole('heading',{name:/失败原因/}).waitFor();assert.ok(!(await desktop.locator('.ops-workspace').innerText()).includes('CANARY'));checks.push('incomplete historical task has honest missing values and redacted failure detail');
