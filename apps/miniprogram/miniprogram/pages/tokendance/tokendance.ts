...
Component({
  data: { email: '', showAccountSettings: false, payments: [] as any[], historyLoaded: false, connected: false, busy: false, state: '', code: '', authorizationUrl: '', amount: '10', balance: '', error: '', notice: '', payment: null as any, attemptId: '' },
  ...
  lifetimes: {
    attached() { (this as any).unsubscribe = subscribeSession(() => { this.stopPolling(); this.setData({ connected: false, busy: false, state: '', code: '', authorizationUrl: '', balance: '', email: '', payments: [], historyLoaded: false, error: '', notice: '', payment: null, attemptId: '' }) }) },
  ...
  methods: {
    ...
    async refresh() { const status = await refreshTokenDanceConnection(); this.setData({ connected: status.connected, email: getCurrentUser()?.email || '' }); if (!getCurrentUser()) this.setData({ error: '请先返回生成页登录图研。' }) },
    async authorize() {
    ...
    async exchange() {
      ...
      try { await accountRequest({ action: 'tokenDanceExchange', state: this.data.state, code }); this.setData({ state: '', authorizationUrl: '', notice: '观猹 TokenDance 已连接，返回生成或精修页即可使用。' }); await this.refresh() }
      catch (error) { this.setData({ error: formatError(error) }) } finally { this.setData({ busy: false }) }
    },
    ...
    async disconnect() { try { await accountRequest({ action: 'tokenDanceDisconnect' }); this.setData({ connected: false, balance: '', payment: null, notice: '已解除图研连接。远端 Key 如需撤销，请到观猹 TokenDance 密钥管理操作。' }); await this.refresh() } catch (error) { this.setData({ error: formatError(error) }) } },
    async queryBalance() {
    ...
    async createPayment() {
      ...
      if (this.data.busy) return
      if (this.data.payments.some(item => ['unknown', 'creating'].includes(item.state)) || (this.data.payment?.status === 'pending' && this.data.payment.expired_at * 1000 > Date.now())) { this.setData({ error: '已有待支付或待核对的订单，请先查看充值记录。' }); return }
    ...
    async recentPayments() {
      try { const result = await accountRequest<{ payments: any[] }>({ action: 'tokenDancePayments' }); this.setData({ historyLoaded: true, payments: result.payments.map(item => ({ ...item, statusText: paymentLabel(item.session?.status || item.state), createdText: item.createdAt ? new Date(item.createdAt).toLocaleString('zh-CN', { hour12: false }) : '' })) }); const row = result.payments.find(item => item.session?.status === 'pending') || result.payments.find(item => item.session); if (row) { this.setData({ payment: row.session, attemptId: row.attemptId }); this.pollPayment() } else this.setData({ notice: result.payments.length ? '订单创建结果尚未确认，请到观猹 TokenDance 核对后再操作。' : '暂无充值记录。' }) } catch (error) { this.setData({ error: formatError(error) }) }
    },
    async queryHistoryPayment(event: WechatMiniprogram.TouchEvent) {
      ...
      if (row?.session) { this.setData({ payment: row.session, attemptId: row.attemptId }); await this.paymentStatus() }
    },
    async paymentStatus() {
      if (!this.data.attemptId || (this as any).statusInFlight) return
      ;(this as any).statusInFlight = true
      try { const result = await accountRequest<{ session: any }>({ action: 'tokenDancePaymentStatus', attemptId: this.data.attemptId }); this.setData({ payment: result.session, payments: this.data.payments.map(item => item.attemptId === this.data.attemptId ? { ...item, session: result.session, statusText: paymentLabel(result.session.status) } : item) }); if (result.session.status === 'paid') { this.setData({ notice: '充值已确认到账，可以返回原任务继续。' }); void this.queryBalance() } if (result.session.status !== 'pending' || result.session.expired_at * 1000 < Date.now()) this.stopPolling() }
