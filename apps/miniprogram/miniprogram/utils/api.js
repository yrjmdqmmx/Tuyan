...
function formatError(error) {
    ...
    if (/ACCOUNT_LIFECYCLE_CLOSED|no longer accepting credential/.test(message))
        return '该账号正在注销或已失效，无法建立新的登录会话。';
    if (/Account deletion is in progress|ACCOUNT_DELETION_IN_PROGRESS/i.test(message))
        return '当前账号正在注销，请打开账号设置查看处理状态。';
    if (/ACCOUNT_DELETION_(PROCESSING|RETRY_SCHEDULED|WAITING_FOR_)/.test(message))
