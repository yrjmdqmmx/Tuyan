export const FEATURED_TEMPLATE_REFERENCE_IDS = ['ref_279', 'ref_281', 'ref_245', 'ref_240', 'ref_295', 'ref_10'] as const
...
export interface FeaturedTemplateCard extends FeaturedTemplate {
  imageUrl: string
  hasImage: boolean
}
...
export function featuredTemplateRequest(): { referenceIds: readonly string[] } {
  return { referenceIds: FEATURED_TEMPLATE_REFERENCE_IDS }
}

export function attachFeaturedTemplateImages(references: unknown[]): FeaturedTemplateCard[] {
  ...
  for (const value of references) {
    ...
    if (id) images.set(id, imageUrl)
  }
  return FEATURED_TEMPLATES.map((template) => {
