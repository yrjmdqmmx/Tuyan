...
function mainModelCanReadImages(provider, model) {
    ...
    if (provider === 'bailian')
        return /qwen3\.8-max|qwen3\.7-plus|qwen3\.5-omni|omni|kimi[\/-]kimi-k3|qwen-?vl|qwen3-?vl|-vl-|qvq/.test(m);
    if (provider === 'gemini')
        return true;
    if (provider === 'openai')
        return /gpt-4|gpt-5|o4|gpt-4o|gpt-4.1/.test(m);
...
function supportedResolutions(provider, _imageModel) {
    if (provider === 'bailian')
        return ['1K', '2K'];
    if (provider === 'gemini')
        return ['1K', '2K'];
