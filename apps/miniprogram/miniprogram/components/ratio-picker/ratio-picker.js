...
Component({
    ...
    data: { expanded: false, visibleOptions: [], total: 0, hasMore: false },
    observers: { 'options, value, expanded'() { this.present(); } },
    methods: {
        present() {
            const options = this.properties.options.filter(item => !item.disabled);
        ...
        select(event) {
            ...
            if (!this.properties.disabled && this.properties.options.some(item => item.value === value && !item.disabled))
