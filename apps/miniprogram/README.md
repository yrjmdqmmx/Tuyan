...
- 普通模式使用单渠道三角色，默认观猹图片模型为 Seedream 5.0 Pro；同渠道各角色可单独选择模型；专业模式支持跨渠道 `modelRoutes`，模型选择器按“API 渠道 → 模型厂商 → 具体模型”分组并支持数百项搜索。
...
本地检查：`npm test`、`npm run check`、`npm run build`。`tests/release-metadata-3.0.1.test.cjs` 同时验证当前 3.5.2 元数据和保留的 3.0.1 历史功能记录。
