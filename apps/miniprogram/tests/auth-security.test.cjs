...
assert.equal(validateChangePassword({ currentPassword: 'a'.repeat(7), newPassword: 'password-123', confirmation: 'password-123' }), 'CURRENT_PASSWORD_TOO_SHORT')
assert.equal(validateChangePassword({ currentPassword: 'a'.repeat(8), newPassword: 'password-123', confirmation: 'password-123' }), '')
assert.equal(validateChangePassword({ currentPassword: 'a'.repeat(128), newPassword: 'password-123', confirmation: 'password-123' }), '')
assert.equal(validateChangePassword({ currentPassword: 'a'.repeat(129), newPassword: 'password-123', confirmation: 'password-123' }), 'CURRENT_PASSWORD_TOO_LONG')
assert.equal(validateChangePassword({ currentPassword: 'current-123', newPassword: '', confirmation: '' }), 'NEW_PASSWORD_REQUIRED')
assert.equal(validateChangePassword({ currentPassword: 'current-123', newPassword: 'short', confirmation: 'short' }), 'PASSWORD_TOO_SHORT')
assert.equal(validateChangePassword({ currentPassword: 'current-123', newPassword: 'password-123', confirmation: '' }), 'PASSWORD_CONFIRMATION_REQUIRED')
assert.equal(validateChangePassword({ currentPassword: 'current-123', newPassword: 'password-123', confirmation: 'password-456' }), 'PASSWORD_CONFIRMATION_MISMATCH')
assert.equal(validateChangePassword({ currentPassword: 'current-123', newPassword: 'password-123', confirmation: 'password-123' }), '')
...
assert.equal(extractAuthErrorCode(toBusinessError(401, { code: 401, error: 'Invalid email or password' })), 'INVALID_EMAIL_OR_PASSWORD')
assert.equal(mapAuthError(toBusinessError(401, { code: 401, error: 'Invalid email or password' })).code, 'INVALID_EMAIL_OR_PASSWORD')
assert.equal(extractAuthErrorCode(toBusinessError(401, { code: 401, error: 'Invalid password' })), 'INVALID_PASSWORD')
assert.equal(mapAuthError(toBusinessError(401, { code: 401, error: 'Invalid password' })).code, 'INVALID_PASSWORD')
assert.equal(extractAuthErrorCode(toBusinessError(400, { code: '  INVALID_PASSWORD  ' })), 'INVALID_PASSWORD')
