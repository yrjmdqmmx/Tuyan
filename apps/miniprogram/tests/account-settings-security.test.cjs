...
test('account security exposes verified state and parent wiring contracts', () => {
  ...
  const wxss = fs.readFileSync(path.join(__dirname, '../miniprogram/components/account-settings/account-settings.wxss'), 'utf8')
  ...
  assert.match(wxml, /输入当前密码并设置新密码。修改成功后，其他设备需要重新登录。/)
  assert.match(wxml, /password maxlength="128" value="\{\{currentPassword\}\}" placeholder="输入当前密码"/)
  assert.match(wxml, /password maxlength="128" value="\{\{newPassword\}\}" placeholder="设置新密码（至少 8 位）"/)
  assert.doesNotMatch(wxml, /8[–到]128 位/)
  assert.match(wxss, /\.logout-button \{[^}]*display:flex;[^}]*align-items:center;[^}]*justify-content:center;/)
