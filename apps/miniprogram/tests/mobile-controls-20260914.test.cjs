...
function sheet(settings,keys={}){const f=loadComponent('components/generation-settings-sheet/generation-settings-sheet.js',{'../../utils/tokendance':{hasTokenDanceConnection:()=>true}});const p=f.instance;p.setData=function(patch,cb){Object.assign(this.data,patch);cb?.()};Object.assign(p.properties,{settings,purpose:'create',apiKeys:keys});p.resetDraft();return f}
test('mixed routes identify Google despite connected TD; direct focus, cancel and save keep shared channel keys coherent',()=>{
 ...
 assert.equal(p.data.routeRows.find(r=>r.role==='main').credentialStatus,'缺少 API Key');assert.equal(p.data.routeRows.find(r=>r.role==='image').credentialStatus,'使用观猹账户授权');assert.match(p.data.credentialSummary,/Google.*API Key/);
