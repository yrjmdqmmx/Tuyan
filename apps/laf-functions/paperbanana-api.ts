...
const modelRegistryVersion = '2026-09-07.v14'
...
async function openRouterProviderRegistry(): Promise<ProviderModelRegistry> {
  ...
  const models = [...entries.values()].map(applyModelLifecycle).filter((model) => model.selectable && model.roles.length > 0).sort((a, b) => {
