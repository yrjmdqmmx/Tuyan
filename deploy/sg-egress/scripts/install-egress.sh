...
if [[ "$mode" == "--dry-run" ]]; then
  ...
  echo "Would bind Squid only to 10.77.0.2:3128 and permit only Hong Kong 10.77.0.1 to the reviewed HTTPS provider hosts."
  exit 0
fi
...
acl approved dstdomain -n api.openai.com generativelanguage.googleapis.com openrouter.ai ark.cn-beijing.volces.com api.anthropic.com api.x.ai external.api.recraft.ai .bfl.ai api.stability.ai api.ideogram.ai api.minimax.io api.mistral.ai api.together.ai api.fireworks.ai queue.fal.run api.replicate.com api.runware.ai tokendance.space
