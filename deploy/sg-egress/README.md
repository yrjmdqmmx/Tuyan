...
3. On Singapore, inspect `sudo scripts/uninstall.sh --host sg --dry-run`, then run `sudo scripts/uninstall.sh --host sg --apply`. This SG mode never removes Hong Kong monitor assets. It stops project units by loaded/active state and stops `pbsg0` plus the exact `10.77.0.2:3128` listener from live kernel/socket state even when a marker or unit file was lost. Any stop failure aborts before configuration deletion; only marked configuration is removed. It restores the package Squid configuration when a backup exists, and does not weaken SSH or delete user data.
