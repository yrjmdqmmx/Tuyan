...
if(operation==="diagnose"){
  ...
  const hash=value=>typeof value==="string"&&/^[a-f0-9]{64}$/.test(value),cny=value=>Number.isFinite(value)&&value>=0,ledger=value=>value&&typeof value==="object"&&!Array.isArray(value)&&["ark","bailian","openrouter"].every(key=>Object.hasOwn(value,key))&&Object.keys(value).every(key=>["ark","bailian","openrouter","replicate"].includes(key))&&Object.values(value).every(cny);
  const canary=value=>value&&typeof value==="object"&&!Array.isArray(value)&&Object.keys(value).sort().join("\\0")===["provider","canonicalModelId","caseId","slotId","status","attemptCount","responseClasses","estimatedCny","actualCny"].sort().join("\\0")&&["bailian","ark","openrouter","replicate"].includes(value.provider)&&[value.canonicalModelId,value.caseId,value.slotId,value.status].every(field=>typeof field==="string"&&field.length>0&&field.length<=200)&&Number.isInteger(value.attemptCount)&&value.attemptCount>=0&&value.attemptCount<=4&&Array.isArray(value.responseClasses)&&value.responseClasses.length===value.attemptCount&&value.responseClasses.every(field=>typeof field==="string"&&field.length>0&&field.length<=128)&&[value.estimatedCny,value.actualCny].every(field=>field===null||cny(field));
  if(typeof data.batchId!=="string"||!data.batchId||!hash(data.manifestHash)||!hash(data.stateHash)||typeof data.status!=="string"||data.status.length>64||![data.pauseReason,data.blockReason].every(value=>value===null||(typeof value==="string"&&value.length<=128))||!ledger(data.providerSpentCny)||!ledger(data.providerUnreconciledCny)||!Number.isInteger(data.revision)||data.revision<0||!Array.isArray(data.providerCanaries)||data.providerCanaries.length>4||!data.providerCanaries.every(canary)||!hash(data.diagnosticHash)||!hash(data.attestationHash))throw new Error("SCIENTIFIC_V2_ADMIN_RESPONSE_SCHEMA_INVALID");
}
