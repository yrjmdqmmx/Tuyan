...
cmp -s \
  <(jq -S '{registryVersion,routeContractVersion,providers:({bailian:.providers.bailian,ark:.providers.ark,openrouter:.providers.openrouter} + (if .providers.replicate then {replicate:.providers.replicate} else {} end))}' "$live_registry_result") \
  <(jq -S '.registry | {registryVersion,routeContractVersion,providers:({bailian:.providers.bailian,ark:.providers.ark,openrouter:.providers.openrouter} + (if .providers.replicate then {replicate:.providers.replicate} else {} end))}' "$authority_path") \
