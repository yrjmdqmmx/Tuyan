...
test('V1 retirement production operator is exact-release, archive-bound, disabled-worker and zero-provider guarded', () => {
  ...
  assert.match(host, /read_env_value "\$core_env" OSS_ACCESS_KEY_ID/u)
  assert.match(host, /read_env_value "\$core_env" OSS_ACCESS_KEY_SECRET/u)
  assert.match(host, /-e PAPERBANANA_V1_RETIREMENT_DELETE_OSS_ACCESS_KEY_ID/u)
  assert.match(host, /-e PAPERBANANA_V1_RETIREMENT_DELETE_OSS_ACCESS_KEY_SECRET/u)
  assert.match(host, /-e "PAPERBANANA_V1_SUMMARY_MODE=\$mode" paperbanana-api node -e/u)
  assert.match(host, /process\.env\.PAPERBANANA_V1_SUMMARY_MODE/u)
  assert.doesNotMatch(host, /node -e '[\s\S]+?'(?: --)? "\$mode" <"\$output"/u)
  ...
  assert.match(worker, /bench\/retirement-probes\/\$\{expectedInventoryHash\}\.capability/u)
  assert.match(worker, /PAPERBANANA_V1_RETIREMENT_DELETE_OSS_ACCESS_KEY_ID/u)
