...
test('review pack bundle is derived offline from one exact completed state with automatic judges fixed to zero', () => {
  ...
  assert.match(source, /createScientificReviewPacket/)
  assert.match(source, /createScientificReviewSourceBindings/)
  assert.match(source, /state[.]status\s*!==\s*["']completed["']/)
  assert.match(source, /operation:\s*["']review_pack["']/)
...
test('current frozen batch has a dedicated zero-provider public render runner with the correct runtime schema', () => {
  ...
  assert.match(source, /-v "\$input_dir\/renderer[.]mjs:\/run\/paperbanana-scientific-v2\/renderer[.]mjs:ro"/)
  assert.match(source, /benchmark-operator node \/run\/paperbanana-scientific-v2\/renderer[.]mjs/)
