...
const remediationProgressWorkflow = fileURLToPath(new URL('../../../.github/workflows/inspect-scientific-v2-remediation-progress.yml', import.meta.url))
...
const persistedWorkerImportWorkflow = fileURLToPath(new URL('../../../.github/workflows/import-scientific-v2-persisted-worker-state.yml', import.meta.url))
...
test('remediation progress inspection exposes the exact persisted transition hash without provider calls', () => {
  const source = readFileSync(remediationProgressWorkflow, 'utf8')
  assert.match(source, /stateTransitionFromHash/)
  assert.match(source, /providerCalls:\s*0/)
  assert.match(source, /inspect-scientific-v2-remediation-zero-provider-calls/)
  assert.doesNotMatch(source, /PAPERBANANA_BENCH_(?:BAILIAN|ARK|OPENROUTER)_API_KEY/)
})
...
test('persisted Worker state import is rebuilt offline from exact protected snapshots without provider access', () => {
  const source = readFileSync(persistedWorkerImportWorkflow, 'utf8')
  assert.match(source, /paperbanana-hk-production[.]lock/)
  assert.match(source, /--network none/)
  ...
  assert.match(source, /state_sha256/)
  assert.match(source, /attestation_result_sha256/)
  assert.match(source, /previous_state_hash/)
  assert.match(source, /expected_state_hash/)
  assert.match(source, /PAPERBANANA_BENCH_REVIEW_SIGNING_SECRET=/)
  assert.match(source, /import-worker-scientific-v2-admin-disabled-worker/)
  assert.match(source, /providerCalls["']?:0/)
  assert.match(source, /install -o 0 -g 0 -m 0600/)
  assert.doesNotMatch(source, /PAPERBANANA_BENCH_(?:BAILIAN|ARK|OPENROUTER)_API_KEY|set -x|printenv|rm -rf/)
})
...
test('blind assignment export keeps mappings on-host and uploads only short-lived public A/B packages', () => {
  ...
  assert.doesNotMatch(source, /batch_id\s*=\s*['"]scientific-v2-['"]\s*\+/)
