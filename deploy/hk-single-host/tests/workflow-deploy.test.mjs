...
test('normal deploy and credential activation share one exact production host lock and workflow concurrency group', () => {
  ...
  const flockIndex = wrapper.search(/flock -x "\$shared_lock_fd"/);
  const installIndex = wrapper.indexOf('install -m 0600');
  const bootstrapIndex = wrapper.indexOf('bootstrap-benchmark.sh');
  const deployApplyIndex = wrapper.search(/deploy\.sh" --apply/);
  assert.ok(flockIndex >= 0, 'shared lock acquisition must exist');
  assert.ok(installIndex >= 0, 'staged image-lock install must exist');
  assert.ok(bootstrapIndex >= 0, 'benchmark bootstrap must exist');
  assert.ok(deployApplyIndex >= 0, 'deploy apply must exist');
  assert.ok(flockIndex < installIndex);
  assert.ok(installIndex < bootstrapIndex);
  assert.ok(bootstrapIndex < deployApplyIndex);
