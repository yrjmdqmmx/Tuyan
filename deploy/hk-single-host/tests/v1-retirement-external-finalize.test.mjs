...
test('externally deleted V1 objects are reverified before exact DB retirement', () => {
  ...
  assert.match(workflow, /v1-retirement-db-receipt\.json/u)
  assert.match(workflow, /jq -n --slurpfile report/u)
  assert.match(workflow, /deleted == \{ releases: 1, runs: 67, samples: 250, judgments: 124, dispatches: 0, publicEvidence: 124 \}/u)
  assert.doesNotMatch(workflow, /dbReceipts|db_result_normalized/u)
