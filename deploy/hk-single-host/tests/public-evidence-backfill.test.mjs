...
const cleanupScript = fileURLToPath(new URL('../scripts/cleanup-stale-public-evidence-inspect.sh', import.meta.url))
const cleanupWorkflow = fileURLToPath(new URL('../../../.github/workflows/cleanup-stale-public-evidence-inspect.yml', import.meta.url))
...
test('public evidence backfill operator is executable, fixed-SHA and zero-generation guarded', () => {
  ...
  assert.match(source, /PAPERBANANA_PUBLIC_EVIDENCE_RESULT_MODE=\$mode/)
  assert.match(source, /PAPERBANANA_PUBLIC_EVIDENCE_RESULT_RELEASE_HASH=\$release_hash/)
  assert.match(source, /process\.env\.PAPERBANANA_PUBLIC_EVIDENCE_RESULT_MODE/)
  assert.doesNotMatch(source, /' "\$mode" "\$release_hash" <"\$result_path"/)
...
test('stale inspect cleanup is exact-process, tree-scoped, protected, and zero-dispatch', () => {
  assert.equal(existsSync(cleanupScript), true)
  assert.equal(statSync(cleanupScript).mode & 0o111, 0o111)
  const scriptSource = readFileSync(cleanupScript, 'utf8')
  assert.match(scriptSource, /cleanup-stale-public-evidence-inspect/)
  assert.match(scriptSource, /backfill-public-evidence\.sh/)
  assert.match(scriptSource, /--mode[\s\S]*inspect/)
  assert.match(scriptSource, /--expected-sha/)
  assert.match(scriptSource, /--release-hash/)
  assert.match(scriptSource, /inspect-public-evidence-disabled-worker/)
  assert.match(scriptSource, /PAPERBANANA_BENCH_ENABLED[\s\S]*false/)
  assert.match(scriptSource, /PAPERBANANA_BENCH_CONCURRENCY[\s\S]*1/)
  assert.match(scriptSource, /collect_descendants/)
  assert.match(scriptSource, /kill -TERM/)
  assert.match(scriptSource, /kill -KILL/)
  assert.doesNotMatch(scriptSource, /pkill|killall|PAPERBANANA_BENCH_ENABLED\s*=\s*true/)

  assert.equal(existsSync(cleanupWorkflow), true)
  const workflowSource = readFileSync(cleanupWorkflow, 'utf8')
  assert.match(workflowSource, /workflow_dispatch/)
  assert.match(workflowSource, /paperbanana-production/)
  assert.match(workflowSource, /cleanup-stale-public-evidence-inspect\.sh/)
  assert.match(workflowSource, /expected_deployed_sha/)
  assert.match(workflowSource, /source_release_hash/)
  assert.doesNotMatch(workflowSource, /generation|judge/i)
})
