...
test('unknown continuation is exact, serial, disabled-gated and tied to the zero-provider reconciliation run', () => {
  ...
  assert.match(workflow, /model_count=.*\.manifest\.models \| length \| select\(\. >= 1 and \. <= 512\)/)
  assert.match(workflow, /--model-count "\$model_count"/)
...
test('unknown reconciliation preserves the original attempt and refuses automatic replay evidence', () => {
  ...
  assert.match(reconciler, /slot\.status = attempt\.attemptIndex === scientificMaxAttempts\(slot\.provider\) \? 'failed' : 'retrying'/)
  assert.match(entry, /existingAudit\.evidence\?\.workflowRunId !== workflowRunId/)
