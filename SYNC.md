...
- [ ] 观猹身份登录：已全文阅读用户补充的官方 Markdown 协议，并核对在线正文与申请表；主要协议缺口已解决，仍待图研身份应用资格/凭据、表单类型映射和回调匹配确认；已识别无邮箱用户与显式绑定的认证改造需求，运行代码未更改，见 [研究与具体缺口](docs/tokendance/watcha-login-research.md)
- [ ] 真实模型生成/精修/充值到账验收、生产部署（本轮未执行付费调用或生产发布）
