...
变更：生产只读复验发现阿里 OSS 对已经存在的确定性公开 WebP 有时仅返回无 status/code 的 `ResponseError`，旧 Worker 只把 `409/FileAlreadyExists` 识别为重复对象，因而在图片已存在且未丢失时误报 runtime failure；同一 SDK 的 buffered `get()` 与 Range `getStream()` 还可能在 HEAD/单字节 Range 均成功时错误重试或在默认 60 秒窗口内中断完整对象流。公开 rendition 写入失败后现统一使用 300 秒、最多 3 次的无 Range 流读取同一内容寻址对象（仅重试零费用的 OSS 只读传输；不会重试 Provider），每次都丢弃不完整字节，并在 header 与逐块读取处硬限制 25 MiB（测试 store 无流接口时才回退 `get()`）；随后逐字节、SHA-256、`image/webp`、immutable cache-control、metadata hash 与 private ACL 复验。只有完全一致（或最小权限凭据精确返回 `403 AccessDenied`）才作为幂等成功，缺失、漂移或公开 ACL 仍失败。不重新生成模型图片、不改对象字节、题目、分辨率、评分、失败/unknown、盲审或发布规则。
