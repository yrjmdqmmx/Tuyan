...
export async function createRefineRuntime({ port = 0, providerDelay = 0 } = {}) {
  const db = memoryDb();
  ...
  legacy.configureRuntimeFetch(async (input, options = {}) => {
  ...
  const backend = { mode: 'node', async call(body) { const data = await invoke(body); return { status: data.code >= 400 ? data.code : 200, data }; }, cachedStatus: () => ({ mode: 'node', ok: true }), ready: async () => ({ ok: true }) };
  const app = express();
  ...
  const auth = {
    async optionalSession(req) { return { user: { id: req.get('x-test-user') || 'refine-owner', email: 'local@example.invalid', emailVerified: true } }; },
  ...
  return {
    baseUrl, db, objects, providerCalls, image, output, legacy, invoke,
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const runtime = await createRefineRuntime({ port: 8791, providerDelay: 1200 });
